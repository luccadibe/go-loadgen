...
This library allows you to test any HTTP, gRPC, or other services based on a simple API:
to test a service, you need to define a client, a data provider, and a collector.
...
This library was created as an alternative to k6 for longer workloads where resource usage
was too high, and as an alternative to ghz which stores all results in memory and writes
them to disk at the end. This library provides full flexibility to implement your own collector.
...
This simple library gives you flexibility to test any service and avoid rewriting the same
executor code each time.
*/
package go_loadgen
